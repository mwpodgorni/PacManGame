...
class Pacman(object):
    ...
    def draw_vectors(self):
        ...
        if self.WANDER_TYPE == 2:
            center = self.position + self.velocity.normalize() * self.ring_distance
            pygame.draw.line(self.screen, WHITE, center, self.displacement, 5)
            pygame.draw.circle(
                self.screen, WHITE, (int(center.x), int(center.y)), self.ring_radius, 1
            )
        else:
            # approach radius
            pygame.draw.circle(
    ...
    def wander_improved(self):
        self.WANDER_TYPE = 2
        future = self.position + self.velocity.normalize() * self.ring_distance
        target = future + pygame.math.Vector2(self.ring_radius, 0).rotate(
            uniform(0, 360)
        )
